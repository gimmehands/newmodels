...
def get_image(data_path, dataset_type):
    image_dict = {}
    path_list = []
    # dataset_type = ['accident', 'dense_traffic', 'fire', 'sparse_traffic']
    dataset_type = ['Fog', 'Rain', 'Sand', 'Snow']
    for train_type in ['train', 'test']:
        for dataset in dataset_type:
            path_list.append(os.path.join(data_path, train_type, dataset))
...
def get_data(data_path, mode, image_dict):
    ...
    class_counts = count(labels)

    # type_names = ['accident', 'dense traffic', 'fire', 'sparse traffic']
    type_names = ['Fog', 'Rain', 'Sand', 'Snow']
    print(f"{mode} contains: {class_counts[0]} {type_names[0]}, {class_counts[1]} {type_names[1]}, {class_counts[2]} {type_names[2]}, {class_counts[3]} {type_names[3]}.")

...
from four_loss import FullContrastiveLoss, SelfContrastiveLoss, CaptionLoss
import json
...
def main(args):
    ...
    valid_class_pre_sum = [0., 0., 0., 0.]
    valid_class_recall_sum = [0., 0., 0., 0.]
    valid_class_f1_sum = [0., 0., 0., 0.]
    ...
    for i in range(K):
        ...
        for epoch in range(args.num_epoch):
            ...
            for j, (train_text, train_image, train_labels) in enumerate(train_loader):
                class_counts = count(train_labels)
                train_text, train_image, train_labels = to_var(train_text), to_var(train_image), to_var(train_labels)

                # Forward + Backward + Optimize
                criterion_full = FullContrastiveLoss(batch_size=train_text.shape[0], class_counts=class_counts)
            ...
            if cur_valid_acc > best_valid_acc:
                ...
                target_names = ['accident', 'dense traffic', 'fire', 'sparse traffic']
                report = metrics.classification_report(valid_y, valid_pred, output_dict=True, target_names=target_names)
                for idx, name in enumerate(target_names):
                    valid_class_pre_sum[idx] += report[name]['precision']
                    valid_class_recall_sum[idx] += report[name]['recall']
                    valid_class_f1_sum[idx] += report[name]['f1-score']
        ...
        results = {
            ...
            'class_precision': [report[name]['precision'] for name in target_names],
            'class_recall': [report[name]['recall'] for name in target_names],
            'class_f1': [report[name]['f1-score'] for name in target_names]
        }
    ...
    # type_names = ['accident', 'dense traffic', 'fire', 'sparse traffic']
    type_names = ['Fog', 'Rain', 'Sand', 'Snow']
    for idx, name in enumerate(type_names):
        print('{} Precision:{:.5f}, {} Recall:{:.5f}, {} F1:{:.5f}'.format(
            name, valid_class_pre_sum[idx] / K, name, valid_class_recall_sum[idx] / K, name,
                  valid_class_f1_sum[idx] / K))
    ...
    results = {
        ...
        'class_precision': [valid_class_pre_sum[idx] / K for idx in range(4)],
        'class_recall': [valid_class_recall_sum[idx] / K for idx in range(4)],
        'class_f1': [valid_class_f1_sum[idx] / K for idx in range(4)]
    }

...
from four_data_loader import load_data
...
def main():
    args = Args(
        seed=0,
        data_path='./datasets/dawn_p2/',
        dataset_type='type_four',  # or 'ds'
        num_epoch=50,
        seq_len=80,
        batch_size=64
    )
    ...
    mlp = MLP(input_dim=input_dim, hidden_dim=256, output_dim=4).to(device)
    ...
    results = {
        ...
        'class_metrics': {
            ...
            'class_3': {
                'precision': precision_per_class[3],
                'recall': recall_per_class[3],
                'f1': f1_per_class[3]
            }
        }
    }

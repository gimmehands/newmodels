...
do
    ...
    do
        for seed in "${seeds[@]}"
        do
            output_path="./result/${dataset_type}_new/seed${seed}/noTE"
            echo "Running with caption_rate: ${caption_rate}, dataset_type: ${dataset_type}, seed: ${seed}"
            python new_main_kfold2.py --data_path ./trafficnet_dataset_v1/ --input_path ./trafficnet_dataset_v1/embedding_inputs --caption_rate ${caption_rate} --seq_len 80 --dataset_type ${dataset_type} --seed ${seed} --output_path ${output_path} --num_epoch 50 --remark "tsne" --need_tsne_data True
